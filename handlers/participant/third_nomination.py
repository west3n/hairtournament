...
class Nomination_third(StatesGroup):
    first_photo = State()
    second_photo = State()
    first_video = State()
    second_video = State()
        ...
        try:
            if user_data[1] == "Champ2023|Econom":
                await bot.send_message(tg_id,
                                       f"<b>{user_data[0]}</b>, вы участвуете в номинации “Короткие волосы”"
...
async def change_media_second(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.first_photo.state)
        photo_path = 'media/second_example.jpg'
        with open(photo_path, 'rb') as photo:
            await call.message.delete()
            await call.message.answer_photo(photo=photo,
...
async def change_media_third(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video,
                                            caption=f"Номинация: 'Короткие волосы'"
                                                    f"\n\nЗагрузите видео №2. Для этого отправьте видео в этот чат"
                                                    f"\n\n<b>Инструкция по съемке:</b> "
                                                    f"\nМодель сидит в кресле с прямой спиной. "
                                                    f"Модель сидит в кресле с прямой спиной. "
                                                    f"Волосы распущены и не убраны за уши. "
                                                    f"Демонстрируется длина волос модели по зоне будущего пробора. "
                                                    f"Для этого оттягиваем прядь волос модели вверх и "
                                                    f"прикладываем линейку к голове модели таким образом, "
                                                    f"чтобы у кожи казался «0». Далее аналогичным образом показываем "
                                                    f"длину"
                                                    f"волос модели на височных частях, оттягивая волосы в бок "
                                                    f"и прикладывая линейку к ним. Центр кадра находится на уровне "
                                                    f"демонстрируемой зоны. Камера закреплена на штативе либо съемку "
                                                    f"производит третье лицо. Камеру держим в перпендикулярном "
                                                    f"положении"
                                                    f"относительно пола. Рекомендованное расстояние от камеры до модели "
                                                    f"до 50 см. Длительность видео от 40 секунд. "
                                                    f"Снимается одним непрерывным дублем."
                                                    f"\n\nРедактировать видео запрещено❌")
            await Nomination_third.next()
    else:
        await state.set_state(Nomination_third.second_photo.state)
        video_path = 'media/first_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video,
        ...
        with open(video_path, 'rb') as video:
            ...
            await call.message.answer_video(video=video,
                                            caption=f"Номинация: 'Короткие волосы'"
                                                    ...
                                                    f"далее приближается камера и снимается процесс капсуляции: "
                                                    f"3 микро и 3 стандартных прядей поштучно (перекапсуляция готовых прядей)."
                                                    f" Демонстрируется техника работы. "
                                                    f"Далее закапсулированные пряди демонстрируются поочередно со всех "
                                                    f"сторон на фоне белого листа бумаги. Камера держится таким образом, "
                                                    f"чтобы руки и щипцы не мешали разглядеть капсулу "
                                                    f"(для правшей - камера слева, для левшей - справа)."
                                                    f" В центре кадра находится капсулируемая прядь."
                                                    f" Камеру держим либо в перпендикулярном положении относительно пола,"
                                                    f" либо с небольшим наклоном в сторону демонстрируемой зоны. "
                                                    f"Камера закреплена на штативе либо съемку производит третье лицо. "
                                                    f"Рекомендованное расстояние от камеры до капсулы до 30 см. "
                                                    f"В центре кадра находится демонстрируемая капсула."
                                                    f"Общая длительность видео до 7 минут. "
                                                    f"Снимается одним непрерывным дублем."
                                                    "\n\nРедактировать видео запрещено❌")
            await Nomination_third.next()
    else:
        await state.set_state(Nomination_third.first_video.state)
        video_path = 'media/second_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video,
...
async def change_media_fifth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.second_video.state)
        video_path = 'media/third_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video,
                                            caption=f"Номинация: 'Короткие волосы'"
                                                    ...
                                                    f"\n\n<b>Инструкция по съемке:</b> "
                                                    f"\nВ начале съемки фиксируется лицо участника чемпионата, "
                                                    f"далее демонстрируется техника наращивания. "
                                                    f"Для демонстрации используются закапсулированные 6 прядей "
                                                    f"(сделанные при демонстрации техники капсуляции, 3 микро и "
                                                    f"3 стандартных пряди)."
                                                    f"Пушковые и короткие волосы, мешающие демонстрации, хорошо "
                                                    f"подколоты."
                                                    f"В центре кадра находится наращиваемая прядь. "
                                                    f"Камеру держим в перпендикулярном положении относительно пола либо "
                                                    f"с небольшим наклоном к демонстрируемой зоне. "
                                                    f"Если вы правша - камера находится слева. "
                                                    f"Если левша, то справа. "
                                                    f"Камера закреплена на штативе либо съемку производит третье лицо. "
                                                    f"Рекомендованное расстояние от камеры до капсулы до 50 см. "
                                                    f"Далее наращенные пряди демонстрируются на фоне белого листа "
                                                    f"бумаги"
                                                    f"(подкладывается под ряд). В центре кадра находятся демонстрируемые капсулы. "
                                                    f"Камеру держим в перпендикулярном положении относительно пола. "
                                                    f"Камера закреплена на штативе либо съемку производит третье лицо. "
                                                    f"Рекомендованное расстояние от камеры до капсулы до 30 см."
                                                    f"Общая длительность видео до 7 минут. Снимается одним "
                                                    f"непрерывным дублем."
                                                    "\n\nРедактировать видео запрещено❌")
            await Nomination_third.next()
...
async def change_media_sixth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video)
            await call.message.answer("Номинация: 'Короткие волосы'"
        ...
        await state.set_state(Nomination_third.third_video.state)
        photo_path = 'media/third_example.jpg'
        with open(photo_path, 'rb') as photo:
            await call.message.delete()
            await call.message.answer_photo(photo=photo,
...
async def change_media_seventh(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.third_photo.state)
        video_path = 'media/fourth_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video)
...
async def change_media_eight(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.fourth_video.state)
        photo_path = 'media/fourth_example.jpg'
        with open(photo_path, 'rb') as photo:
            await call.message.delete()
            await call.message.answer_photo(photo=photo,
...
async def change_media_ninth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.fourth_photo.state)
        video_path = 'media/fifth_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video,
...
async def change_media_tenth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.fifth_video.state)
        photo_path = 'media/fifth_example.jpg'
        with open(photo_path, 'rb') as photo:
            await call.message.delete()
            await call.message.answer_photo(photo=photo,
...
async def change_media_eleventh(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.fifth_photo.state)
        photo_path = 'media/sixth_example.jpg'
        with open(photo_path, 'rb') as photo:
            await call.message.delete()
            await call.message.answer_photo(photo=photo,
...
async def change_media_twelfth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.sixth_photo.state)
        video_path = 'media/sixth_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video,
...
async def change_media_thirteenth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.sixth_video.state)
        video_path = 'media/seventh_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video,
...
async def change_media_fourteenth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.seventh_video.state)
        video_path = 'media/eighth_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video)
...
async def change_media_fifteenth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.eighth_video.state)
        video_path = 'media/ninth_example.mp4'
        with open(video_path, 'rb') as video:
            await call.message.delete()
            await call.message.answer_video(video=video,
...
async def change_media_sixteenth(call: types.CallbackQuery, state: FSMContext):
    if call.data == "next_angle":
        ...
        await state.set_state(Nomination_third.ninth_video.state)
        photo_path = 'media/seventh_example.jpg'
        with open(photo_path, 'rb') as photo:
            await call.message.delete()
            await call.message.answer_photo(photo=photo,
        ...
        async with state.proxy() as data:
            ...
            await call.bot.send_message(chat_id=decouple.config('GROUP_ID'),
                                        ...
                                        text=f"Фотографии и видеозаписи участницы {user_data[0]}"
                                             f"\n\nНоминация: Короткие волосы")
            await state.finish()
    else:
        await state.set_state(Nomination_third.seventh_photo.state)
        photo_path = 'media/eighth_example.jpg'
        with open(photo_path, 'rb') as photo:
            await call.message.delete()
            await call.message.answer_photo(photo=photo,
...
def register(dp: Dispatcher):
    dp.register_message_handler(handle_first_photo, content_types=['photo'], state=Nomination_third.first_photo)
    dp.register_callback_query_handler(change_media_first, state=Nomination_third.first_photo)
    dp.register_message_handler(handle_second_photo, content_types=['photo'], state=Nomination_third.second_photo)
    dp.register_callback_query_handler(change_media_second, state=Nomination_third.second_photo)
    dp.register_message_handler(handle_first_video, content_types=['video'], state=Nomination_third.first_video)
    dp.register_callback_query_handler(change_media_third, state=Nomination_third.first_video)
    dp.register_message_handler(handle_second_video, content_types=['video'], state=Nomination_third.second_video)
    dp.register_callback_query_handler(change_media_fourth, state=Nomination_third.second_video)
    dp.register_message_handler(handle_third_video, content_types=['video'], state=Nomination_third.third_video)
    dp.register_callback_query_handler(change_media_fifth, state=Nomination_third.third_video)
    dp.register_message_handler(handle_third_photo, content_types=['photo'], state=Nomination_third.third_photo)
    dp.register_callback_query_handler(change_media_sixth, state=Nomination_third.third_photo)
    dp.register_message_handler(handle_fourth_video, content_types=['video'], state=Nomination_third.fourth_video)
    dp.register_callback_query_handler(change_media_seventh, state=Nomination_third.fourth_video)
    dp.register_message_handler(handle_fourth_photo, content_types=['photo'], state=Nomination_third.fourth_photo)
    dp.register_callback_query_handler(change_media_eight, state=Nomination_third.fourth_photo)
    dp.register_message_handler(handle_fifth_video, content_types=['video'], state=Nomination_third.fifth_video)
    dp.register_callback_query_handler(change_media_ninth, state=Nomination_third.fifth_video)
    dp.register_message_handler(handle_fifth_photo, content_types=['photo'], state=Nomination_third.fifth_photo)
    dp.register_callback_query_handler(change_media_tenth, state=Nomination_third.fifth_photo)
    dp.register_message_handler(handle_sixth_photo, content_types=['photo'], state=Nomination_third.sixth_photo)
    dp.register_callback_query_handler(change_media_eleventh, state=Nomination_third.sixth_photo)
    dp.register_message_handler(handle_sixth_video, content_types=['video'], state=Nomination_third.sixth_video)
    dp.register_callback_query_handler(change_media_twelfth, state=Nomination_third.sixth_video)
    dp.register_message_handler(handle_seventh_video, content_types=['video'], state=Nomination_third.seventh_video)
    dp.register_callback_query_handler(change_media_thirteenth, state=Nomination_third.seventh_video)
    dp.register_message_handler(handle_eighth_video, content_types=['video'], state=Nomination_third.eighth_video)
    dp.register_callback_query_handler(change_media_fourteenth, state=Nomination_third.eighth_video)
    dp.register_message_handler(handle_ninth_video, content_types=['video'], state=Nomination_third.ninth_video)
    dp.register_callback_query_handler(change_media_fifteenth, state=Nomination_third.ninth_video)
    dp.register_message_handler(handle_seventh_photo, content_types=['photo'], state=Nomination_third.seventh_photo)
    dp.register_callback_query_handler(change_media_sixteenth, state=Nomination_third.seventh_photo)
    dp.register_message_handler(handle_eighth_photo, content_types=['photo'], state=Nomination_third.eighth_photo)
    dp.register_callback_query_handler(finish_nomination, state=Nomination_third.eighth_photo)
